...
import type { IChartContext, IChartOption, IChartSnapshot } from '@univerjs/sheets-chart';
...
export interface IBarSettingConfig extends ISettingConfig {
    subType?: 'normal' | 'smooth' | 'step';
}

const prefixCls = Tools.getPrefixCls('bar-setting');

export function BarSetting() {
    ...
    const onApplySubType = useCallback((e: 'column' | 'bar') => {
        const option: IChartOption = e === 'bar' ? { xAxis: { type: 'value' }, yAxis: { type: 'category' } } : { xAxis: { type: 'category', axisLabel: { rotate: 60, hideOverlap: true, interval: 0 } }, yAxis: { type: 'value' } };
        model.onApplyWithOpts(merge({}, snapshot, { option }), { replaceMerge: ['xAxis', 'yAxis'] });
    }, [snapshot, model]);
...
registerComponent('chart-setting.bar', BarSetting);
