...
export function drawDiagonalLineByBorderType(ctx: UniverRenderingContext, type: BORDER_LTRB, position: IPosition) {
    ...
    let drawEndY = 0;
    const { startX, startY, endX, endY } = position;
    if (type === BORDER_LTRB.TL_BR) {
        drawStartX = startX;
        drawStartY = startY;
        drawEndX = endX;
        drawEndY = endY;
    } else if (type === BORDER_LTRB.TL_BC) {
        drawStartX = startX;
        drawStartY = startY;
        drawEndX = (startX + endX) / 2;
        drawEndY = endY;
    } else if (type === BORDER_LTRB.TL_MR) {
        drawStartX = startX;
        drawStartY = startY;
        drawEndX = endX;
        drawEndY = (startY + endY) / 2;
    } else if (type === BORDER_LTRB.BL_TR) {
        drawStartX = startX;
        drawStartY = endY;
        drawEndX = endX;
        drawEndY = startY;
    } else if (type === BORDER_LTRB.ML_TR) {
        drawStartX = startX;
        drawStartY = (startY + endY) / 2;
        drawEndX = endX;
        drawEndY = startY;
    } else if (type === BORDER_LTRB.BC_TR) {
